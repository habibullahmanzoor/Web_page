...
def get_latest_pubs(n=5):
    try:
        ...
        # normalise to dicts with same keys
        return [{"title": p["title"], "year": p.get("year"), "venue": p.get("venue", ""), "link": None}
                for p in pubs[:n]]
...
with c1:
    ...
    st.image(str(PHOTO) if PHOTO.exists() else "https://via.placeholder.com/220", use_container_width=True)
...
with c3:
    ...
    # three compact stat cards in a grid
    ...
    st.markdown('</div>', unsafe_allow_html=True)  # end .metrics-wrap

    # Remove the journals/conferences/chapters line entirely.
...
with tabs[0]:
    ...
    bio_text = load_bio()
    if bio_text:
        # Build the HTML outside the f-string to avoid backslashes in the {...}
        bio_html = bio_text.replace("\n\n", "</p><p>").replace("\n", "<br>")
        st.markdown(
            f"<div class='card'><p>{bio_html}</p></div>",
            unsafe_allow_html=True
        )
    else:
        st.markdown("""
            <div class='card'>
              <p class='small'>No biography found. Create <code>static/biography.txt</code> to add your bio.</p>
            </div>
        """, unsafe_allow_html=True)
...
# ---- Publications (selected first, then auto-updated latest) ----
with tabs[7]:
    ...
    @st.cache_data(ttl=60*60*12, show_spinner=False)  # refresh every 12h
    def get_latest_pubs():
        return scholar_scraper.fetch_latest_publications(SCHOLAR_URL, count=5)

    st.markdown("### Latest Publications (auto-updated)")
    latest_pubs = get_latest_pubs()
    if latest_pubs:
